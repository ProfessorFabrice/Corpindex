...
def calc(index):
	for f in index:
		...
		for t in cq.cqpl(query):
			if feature != "f":
				m1 = cq.getElement(t[0])[1][0][feature]
				m2 = cq.getElement(t[1])[1][0][feature]
			else:
				m1 = cq.getElement(t[0])[0]
				m2 = cq.getElement(t[1])[0]
...
def calculTest(dicoC,infos):
	...
	for m1 in dicoC:
		...
		for m2 in dicoC[m1]:
			val = dicoC[m1][m2]
			if ecarttype != 0:
				res.append([str((val - infos[m1]["moyenne"])/ecarttype),m1+"\t"+m2]) 
	return res
