...
class Tokenizer(object):
	...
	def t_MOT(self,t):
		...
		for elt in listeInfos:
			elt["level"] = '0'
		t.value = [t.value,listeInfos]
		#print("\t","mot=",mot,"traits=",self.dicoMs.get(mot))
		return t
...
if __name__ == '__main__':
	...
	t.readMs()
	t.readMc()	
	t.init("""           Comme stipulé dans l'accord de paix, les ex-combattants peuvent recevoir un soutien financier supplémentaire du gouvernement national et de la communauté internationale pour entreprendre des projets productifs ou construire des logements individuels ou collectifs. Les projets sont montés par les ex-guérilleros avec le soutien du Ministère du Commerce, de l'Industrie et du Tourisme (MinCIT) et sont évalués par le Conseil National de Réinsertion (CNR)<a class="endnotecall" href="#ftn4" id="bodyftn4">iv</a>. Ce comité estime la faisabilité des projets et est chargé d'approuver et de débourser l'aide financière pour ceux qui répondent aux exigences techniques, financières et légales stipulées par l'ARN<a class="endnotecall" href="#ftn5" id="bodyftn5">v</a>. Ceux qui sont approuvés reçoivent un soutien financier et technique pour leur mise en œuvre (8 millions de peso colombien, soit environ 2200 euros par personne impliquée). """)
