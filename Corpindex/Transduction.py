...
class Transduction(object):
	...
	def __init__(self,nomFic="",verb=False):
		self.cqpl = []
		self.action = []
		self.nomFic = nomFic
	...
	# faut tout réécrire..., c'est le bazar
	#
	...
	def anaTrans(self,arbre,select=True):
		...
		if noeud != None:
			if arbre[0] == 46:  # mot quelconque (*)
				...
				#op1 = self.anaTrans(noeud,arbre[1])
				#matchN1 = self.lsti
				#op2 = self.anaTrans(noeud,arbre[2])
				op2 = self.anaTrans(arbre[2],select)
				#self.matchNoeud = inter(matchN1,self.lsti)
				return op1 and op2
				...
				#op2 = self.anaTrans(noeud,arbre[2])
				op2 = self.anaTrans(arbre[2],select)
				#self.matchNoeud = union(matchN1,self.lsti)
				return op1 or op2
...
if __name__ == '__main__':
	...
	c = Transduction()
	#c.addRules('[f~"^[A-Z]"/r="debut"][*]{0,15}[f="?"/r="fin"]')
	#c.addRules('[l="donner"/emploi="s1"][*]{0,3}[f="confiture"][f="cochons"]')
	#c.addRules('[l="donner"/emploi="s1"][*]{0,3}[f="confiture"]')
	c.addRules('[f="["/*][*]{0,3}[f="]"/*]')
	n=c.ltt[0][0]
	t=c.ltt[0][1]
