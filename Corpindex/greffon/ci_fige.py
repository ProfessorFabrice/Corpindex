...
class Kwicldi(object):
	...
	def traite(self,conc,pre=""):
		...
		pv = conc.getPivot()
		#if self.att =='f':
		pred = pv[self.pp].getFeat(self.att)
		arg = pv[self.pa].getFeat(self.att)
		#else:
		#	pred = pv[self.pp][1][0][self.att]
		#	arg = pv[self.pa][1][0][self.att]
		self.lP[pred] = 1
		self.lA[arg] = 1
